...
def main():
    ...
    elo = build_elo_end_features(regular)  # one row per (Season, TeamID) with EloEnd
    ...
    # Fill missing SeedLogs with the log of 17
    features["SeedLog"] = features["SeedLog"].fillna(np.log(17))
    ...
    # --- NEW: Extract and print feature importance ---
    # 1. Grab the actual Logistic Regression model out of the pipeline
    lr_model = model.named_steps["clf"]

    # 2. Pair the feature names with their learned weights
    feature_names = X.columns
    coefficients = lr_model.coef_[0]

    # 3. Build a clean DataFrame to display them
    ...
    # 4. Sort by the highest absolute impact to see the top drivers
    importance_df = importance_df.sort_values(by="Absolute_Impact", ascending=False).reset_index(drop=True)
