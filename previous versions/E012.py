...
def build_team_season_detailed_features(regular_detailed: pd.DataFrame) -> pd.DataFrame:
    ...
    def get_winner_or_loser(pd, is_winner):
        ...
        return pd.DataFrame({
            ...
            # offense box score
            "FGM": df[f"{id_char}FGM"],
            "FGA": df[f"{id_char}FGA"],
            "FGM3": df[f"{id_char}FGM3"],
            "FGA3": df[f"{id_char}FGA3"],
            "FTM": df[f"{id_char}FTM"],
            "FTA": df[f"{id_char}FTA"],
            "OR": df[f"{id_char}OR"],
            "DR": df[f"{id_char}DR"],
            "Ast": df[f"{id_char}Ast"],
            "TO": df[f"{id_char}TO"],
            "Stl": df[f"{id_char}Stl"],
            "Blk": df[f"{id_char}Blk"],
            "PFoul": df[f"{id_char}PF"],

            # pace denominators
            "Poss": df[f"{id_char}_Poss"],
            "OppPoss": df[f"{opp_char}_Poss"],

            # opponent stats (for defensive allowed features / reb denominators)
            "OppFGM": df[f"{opp_char}FGM"],
            "OppFGA": df[f"{opp_char}FGA"],
            "OppFGM3": df[f"{opp_char}FGM3"],
            "OppFGA3": df[f"{opp_char}FGA3"],
            "OppFTM": df[f"{opp_char}FTM"],
            "OppFTA": df[f"{opp_char}FTA"],
            "OppOR": df[f"{opp_char}OR"],
            "OppDR": df[f"{opp_char}DR"],
...
def main():
    ...
    features_detailed["RankLog"] = features_detailed["RankLog"].fillna(
        100 - 4 * np.log(175.0 + 1) - 175.0/22)
    ...
    final_pred = (0.5 * pred_comp) + ((1 - 0.5) * pred_det)
