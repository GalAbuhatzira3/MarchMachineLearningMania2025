    ...
    for excluded_season in seasons:
        ...
        for stem in unique_stems:
            ...
            for _ in range(n_repeats):
                ...
                for col in related_cols: X_perm[col] = X_val.loc[idx, col].values
...
def plot_importance(imp_df, title):
    ...
    plot_df = imp_df[imp_df["avg_delta"] > 0].sort_values("avg_delta", ascending=True).tail(25)
    ...
    plt.barh(plot_df["feature"], plot_df["avg_delta"], xerr=plot_df["std_delta"], color='skyblue', edgecolor='black')
    plt.title(f"Permutation Importance: {title}")
    plt.xlabel("Increase in Error (Higher = More Important)")
    plt.tight_layout()
    plt.show()  # This ensures the plot window actually opens

def plot_importance(imp_df, title):
    plt.figure(figsize=(10, 12))
    # Filter for features that actually have a positive impact on the error
    plot_df = imp_df[imp_df["avg_delta"] > 0].sort_values("avg_delta", ascending=True).tail(30)

    plt.barh(plot_df["feature"], plot_df["avg_delta"], xerr=plot_df["std_delta"],
             color='skyblue', edgecolor='black', alpha=0.8)

    plt.title(f"Grouped Permutation Importance: {title}", fontsize=14)
    plt.xlabel("Mean Increase in Error when Shuffled (Higher = More Important)", fontsize=12)
...
MODEL_THRESHOLDS = {
    'lr': 0.001,      # Prune noise for linear stability
    'xgb': 0.01,      # scale is in 'points', needs higher threshold
    'xgb_bin': 0.0003 # Allow more features for tree interactions
}
...
def get_prediction_from_model(is_detailed, to_subtract, model_name, features_to_keep=None):
    ...
    if features_to_keep is not None:
        keep_cols = []
        for c in X.columns:
            # STRIP PREFIX: Check if the base name is in our keep list
            base_name = c[2:] if (c.startswith("A_") or c.startswith("B_")) else c
            if base_name in features_to_keep or c in ["Season", "IsMen"]:
                keep_cols.append(c)
        X = X[keep_cols] # Now X and X_test will have the same columns
...
def main():
    ...
    features = ['EloEnd', 'Quality', 'OffEff', 'OppScore', 'TS', 'Pace', 'PF_Rate', 'eFG', 'Def_eFG', 'Score', 'FTR',
                'Def_FTR', 'NetEff', 'DefEff', 'Seed_Gender_Interact', 'BLK_Rate', 'AST_Rate', '3P%', 'Def_3P%', 'TOV'
                'W', '3PAr', 'ORB', 'Road_WinPct', 'RegDiff']
    ...
    features = ['EloEnd', 'Seed', 'Quality', 'Seed_Gender_Interact', 'ORB', 'OffEff', 'NetEff', 'OppScore', 'AST_Rate',
                'Score']
    ...
    features = ['Seed', 'EloEnd', 'Quality', 'FTR', 'Seed_Gender_Interact', 'OffEff', 'Score', 'BLK_Rate']
    ...
    #out["Pred"] = np.where(out["Pred"] >= 0.75, 1.0, out["Pred"])
    #out["Pred"] = np.where(out["Pred"] <= 0.25, 0.0, out["Pred"])
