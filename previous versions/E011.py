...
def get_massey() -> pd.DataFrame:
    # Safely load (Kaggle recently added WMasseyOrdinals for newer women's years, so we check for both)
    dfs = []
    for file in ["MMasseyOrdinals.csv", "WMasseyOrdinals.csv"]:
        path = DATA_DIR / file
        if path.exists():
            dfs.append(pd.read_csv(path))

    if not dfs:
        return pd.DataFrame(columns=["Season", "TeamID", "RankLog"])

    massey = pd.concat(dfs, ignore_index=True)

    # 1. Take ONLY the final pre-tournament rankings (Day 133)
    massey = massey[massey["RankingDayNum"] == 133].copy()

    # 2. Filter for only the elite, highly-predictive analytic models
    elite_systems = ["POM", "SAG", "MOR"]
    massey = massey[massey["SystemName"].isin(elite_systems)]

    # 3. Get the median rank among the elite systems
    consensus = massey.groupby(["Season", "TeamID"], as_index=False).agg(
        MedianRank=("OrdinalRank", "median")
    )

    # 4. Log-transform the rank (crucial for linear models)
    consensus["RankLog"] = np.log(consensus["MedianRank"])
...
def main():
    ...
    features_detailed["RankLog"] = features_detailed["RankLog"].fillna(np.log(175.0))
    ...
    for i, (tr_idx, va_idx) in enumerate(gkf.split(X_det, y_det, groups=groups_det), 1):
        ...
        # To predict on validation games, we slice the compact columns directly out of X_det
    ...
    # --- THE ENSEMBLE ---
    # Automatically apply the best math we just discovered!
