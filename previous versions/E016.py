...
def predict(is_detailed, models, calibrator, X, model_name, to_subtract):
...
def get_prediction_from_model(is_detailed, to_subtract, model_name):
    X, y = get_training_set(is_detailed, to_subtract)
    models, calibrator = train(X, y, model_name)
    out = predict(is_detailed, models, calibrator, X, model_name, to_subtract)
    return out
...
def main():
    ...
    #X, y = get_training_set(is_detailed)

    #train_df = X.copy()
    #train_df["TourDiff"] = y["TourDiff"].values
    #cv_brier, imp_df = rank_features_by_brier_perm_importance(train_df, n_splits=10, n_repeats=5)
    #print("CV Brier:", cv_brier)
    #print(imp_df.head(30))

    # --------------------------------- Prediction & Submission ------------------------------------------------------ #
    out_comp = get_prediction_from_model(is_detailed=False, to_subtract=True, model_name="lr")
    out_det = get_prediction_from_model(is_detailed=True, to_subtract=True, model_name="lr")
    out_xg = get_prediction_from_model(is_detailed=True, to_subtract=False, model_name="xgb")

    out = out_comp.copy()
    out["Pred"] = 0.3 * out_comp["Pred"] + 0.2 * out_det["Pred"] + 0.5 * out_xg["Pred"]
