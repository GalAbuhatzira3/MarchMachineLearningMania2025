...
def build_elo_end_features(regular: pd.DataFrame, k: float = 20.0, base: float = 1500.0) -> pd.DataFrame:
    # This functions runs through the regular season games in chronological order (per season) and maintains a
    # rating per team (and save it as an attribute. i.e. table of [Season, TeamID, EloEnd])

    # [1] Take only needed columns and sort them:
    # We only need: which season it is, when the game happened, who won and who lost:
    games = regular[["Season", "DayNum", "WTeamID", "LTeamID"]].copy()
    # We sort by (Season, DayNum) so that we proceed the season from start -> end. And then next season start -> end:
    games = games.sort_values(["Season", "DayNum"], kind="mergesort")

    # This matters because Elo is an "updating over time" rating

    # [2] Define the expected win probability formula:
    #   ra == rb -> expected = 0.5
    #   ra > rb -> expected > 0.5
    #   ra < rb -> expected < 0.5
    # intuition: it converts rating difference into "how likely A is to win"
    def expected(a_rating: float, b_rating: float) -> float:
        return 1.0 / (1.0 + 10 ** ((b_rating - a_rating) / 400.0))

    # [3] prepare containers:
    rows = []  # will collect the final output rows we return
    current_season = None  # will track when we move from one season to the next
    ratings: dict[int, float] = {}  # ratings[TeamID] = current Elo rating

    # [4] loop through games in time order
    for season, day, winner, loser in games.itertuples(index=False):  # each iteration is one game
        # On new season: save previous season ratings and reset
        if current_season is None or season != current_season:
            if current_season is not None:
                for tid, r in ratings.items():
                    # before resetting, we save all final ratings of the previous season into rows.
                    rows.append((current_season, tid, r))
            current_season = season  # switch to the new season
            ratings = {}  # reset ratings

        # If a team hasn't appeared yet this season - start with base = 1500. otherwise, use its current rating:
        winner_rating = ratings.get(winner, base)
        loser_rating = ratings.get(loser, base)

        # Compute expected scores
        # expected probability that the w team would win, given ratings.
        expected_winner = expected(winner_rating, loser_rating)
        # expected probability that the loser team would win, given ratings (complement of ew).
        expected_loser = 1.0 - expected_winner

        # Elo update rule: R_new = R_old + K * (S - E) where
        #   S = actual score (1 for win, 0 for loss)
        #   E = expected score (probability of winning)
        # Intuitively: if the favorite wins -> small gain. If the underdog wins -> big gain.
        ratings[winner] = winner_rating + k * (1.0 - expected_winner)  # for winner: S = 1
        ratings[loser] = loser_rating + k * (0.0 - expected_loser)  # for loser: S = 0
...
def build_training_from_tournament(tournament: pd.DataFrame, feats: pd.DataFrame):
    ...
    base = [
        "WinPct", "PF_perG", "PA_perG", "Diff_perG", "G", "EloEnd", "SeedStrength", "HasSeed",
        "eFG", "FTR", "TOV", "ORB", "DRB", "Pace", "NetEff",
        "3PAr", "3P%", "FT%", "TS",
        "AST_Rate", "STL_Rate", "BLK_Rate", "PF_Rate",
        "Def_eFG", "Def_3PAr", "Def_3P%", "Def_FTR",
...
def get_regular_detailed():
    def extract_regular_detailed_from_csv(csv: str, min_season: int) -> pd.DataFrame:
        regular_columns = [
            "Season", "DayNum",
        ...
        regular_detailed = read_csv(csv, usecols=regular_columns)
        # keep only regular season games:
        regular_detailed = regular_detailed[regular_detailed["DayNum"] <= DAYS_IN_REGULAR_SEASON]
        # load only the columns we need:
        regular_detailed = regular_detailed[regular_detailed["Season"] >= min_season]
        return regular_detailed

    men_regular = extract_regular_detailed_from_csv(MEN_REG_DET, MEN_MIN_SEASON)
    women_regular = extract_regular_detailed_from_csv(WOMEN_REG_DET, WOMEN_MIN_SEASON)
    regular_detailed = pd.concat([men_regular, women_regular], ignore_index=True)

    print(f"Regular season detailed loaded! Loaded games: {len(regular_detailed):,}")
    return regular_detailed
...
def main():
    ...
    regular = get_regular()
    regular_detailed = get_regular_detailed()
    ...
    elo = build_elo_end_features(regular, k=20.0, base=1500.0)  # one row per (Season, TeamID) with EloEnd
