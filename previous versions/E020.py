...
def get_glm_quality_feature(
    cache: bool = True,
    cache_file: str = "glm_quality.csv",
    verbose: bool = True,
) -> pd.DataFrame:
    """
    Per-season (and per gender) GLM rating:
        PointDiff ~ -1 + T1_TeamID + T2_TeamID
    Returns: Season, TeamID, Quality

    Notes:
    - Uses regular season compact results only.
    - Builds relevant set st = tourney teams + non-tourney teams that beat a tourney team.
    - Collapses all 'irrelevant' opponents into TeamID=0000.
    - Centers Quality within (Season, IsMen) to make it stable.
    """
    import re
    ...
    men_seeds = read_csv(MEN_SEEDS, usecols=["Season", "TeamID"]).copy()
    men_seeds["IsMen"] = 1
    wom_seeds = read_csv(WOMEN_SEEDS, usecols=["Season", "TeamID"]).copy()
    wom_seeds["IsMen"] = 0
    seeds = pd.concat([men_seeds, wom_seeds], ignore_index=True)

    # --- regular compact (we only need scores + team ids) ---
    usecols = ["Season", "WTeamID", "WScore", "LTeamID", "LScore"]
    men_reg = read_csv(MEN_REG, usecols=usecols).copy()
    men_reg["IsMen"] = 1
    wom_reg = read_csv(WOMEN_REG, usecols=usecols).copy()
    wom_reg["IsMen"] = 0
    reg = pd.concat([men_reg, wom_reg], ignore_index=True)

    def extract_t1_teamid(param_name: str) -> int | None:
        # patsy names look like: "T1_TeamID[1181]"  (sometimes also "T1_TeamID[T.1181]" depending on encoding)
        m = re.search(r"T1_TeamID\[(?:T\.)?(\d+)\]", param_name)
    ...
    for s in seasons:
        for is_men in (0, 1):
            # min available seasons (women compacts start later)
            if is_men == 0 and s < 1998:
                continue
            if is_men == 1 and s < 1985:
                continue

            tourney = set(seeds.loc[(seeds["Season"] == s) & (seeds["IsMen"] == is_men), "TeamID"].astype(int))
            if not tourney:
                continue

            reg_s = reg[(reg["Season"] == s) & (reg["IsMen"] == is_men)]
            if reg_s.empty:
                continue

            # non-tourney teams that beat a tourney team at least once
            beaters = set(
                reg_s.loc[(~reg_s["WTeamID"].isin(tourney)) & (reg_s["LTeamID"].isin(tourney)), "WTeamID"].astype(int)
            )
            st = tourney | beaters

            # build symmetric dataset (winner perspective + flipped loser perspective)
            a = reg_s[["WTeamID", "LTeamID", "WScore", "LScore"]].copy()
            a.columns = ["T1", "T2", "S1", "S2"]
            a["PointDiff"] = a["S1"] - a["S2"]

            b = reg_s[["LTeamID", "WTeamID", "LScore", "WScore"]].copy()
            b.columns = ["T1", "T2", "S1", "S2"]
            b["PointDiff"] = b["S1"] - b["S2"]

            dt = pd.concat([a[["T1", "T2", "PointDiff"]], b[["T1", "T2", "PointDiff"]]], ignore_index=True)

            # keep rows where at least one side is relevant; collapse the other side to 0000
            dt = dt.loc[dt["T1"].isin(st) | dt["T2"].isin(st)].copy()
            if dt.empty:
                continue

            dt.loc[~dt["T1"].isin(st), "T1"] = 0
            dt.loc[~dt["T2"].isin(st), "T2"] = 0

            dt["T1_TeamID"] = dt["T1"].astype(int).astype(str).str.zfill(4)
            dt["T2_TeamID"] = dt["T2"].astype(int).astype(str).str.zfill(4)
            ...
            try:
                ...
                if verbose:
                    print(f"[GLMQuality] failed season={s} is_men={is_men}: {e}")
                continue

            for pname, val in glm.params.items():
                tid = extract_t1_teamid(pname)
                if tid is None or tid == 0:
                    continue
                out_rows.append((int(s), int(tid), int(is_men), float(val)))
...
def get_features(is_detailed):
    ...
    reg_season = reg_season.merge(get_glm_quality_feature(), on=["Season", "TeamID"], how="left")
    reg_season["Quality"] = reg_season["Quality"].fillna(0.0)
...
def plot(skeleton):
    tmpmean = skeleton.pivot_table(columns="IsMen", index="EloEnd", values="TourDiff",aggfunc="mean").sort_index().ffill()
    tmpstd = skeleton.pivot_table(columns="IsMen", index="EloEnd", values="TourDiff",aggfunc="std").sort_index().ffill().fillna(0)
...
def train(X, y, model_name):
    ...
    for excluded_season in seasons:
        ...
        if model_name == "xgb":
            ...
            d_train = xgb.DMatrix(X_train, label=y_train_raw)
            model = xgb.train(params=param_margin, dtrain=d_train, num_boost_round=num_rounds)
            models[excluded_season] = model

            d_val = xgb.DMatrix(X_val, label=y_val_raw)
            margins = model.predict(d_val)
            ...
            model = xgb.train(params=param_bin, dtrain=d_train, num_boost_round=num_rounds)
            models[excluded_season] = model

            d_val = xgb.DMatrix(X_val, label=y_val)
            probs = model.predict(d_val)
            ...
            print(f"Excluded season: {excluded_season} LogLoss: {ll:.5f}")
    ...
    print(f"LR OOF LogLoss: {log_loss(labels, probs):.5f}")
...
def main():
    ...
    good_features = imp_df[imp_df["delta_brier"] > 0.001]["feature"].tolist()
    good_features = list(set(good_features + ["Quality"]))
    ...
    print("\n--- Training Detailed XGBoost Binary (All Features) ---")
    ...
    # Meta features: [LR_prob, XGB_prob]
    meta_feats = np.column_stack([oof_det, oof_xg, oof_xg_bin])

    # Fit two meta-models (Men/Women) on OOF predictions to directly optimize logloss
    ...
    meta_women.fit(meta_feats[wom_mask], y_targets[wom_mask])

    oof_meta = np.empty_like(y_targets, dtype=float)
    oof_meta[men_mask] = meta_men.predict_proba(meta_feats[men_mask])[:, 1]
    oof_meta[wom_mask] = meta_women.predict_proba(meta_feats[wom_mask])[:, 1]
    ...
    # Note: ID format is SSSS_XXXX_YYYY and XXXX is the LOWER TeamID (A_TeamID in our parsing)
    season_sub, teamA_sub, teamB_sub = parse_submission_ids(out)
    is_men_sub = (teamA_sub < 3000).astype(int)

    meta_sub = np.column_stack([out_det["Pred"].values, out_xg["Pred"].values, out_xg_bin["Pred"].values])

    final_pred = np.empty(len(out), dtype=float)
    men_mask_sub = (is_men_sub == 1)
    wom_mask_sub = ~men_mask_sub
