...
def train(X, y, model_name):
    ...
    Returns:
        ...
        calibrator: fitted calibrator for xgb margins (or None for lr)
    ...
    # Convert aligned series to arrays in X order
    ...
    if model_name == "xgb":
        ...
        print(f"OOF XGB calibrated LogLoss: {log_loss(labels, np.clip(probs, 1e-6, 1-1e-6)):.5f}")

        return models, calibrator, probs, labels

    else:
        probs = oof_pred.astype(float).values
        print(f"Average Brier: {float(np.mean(fold_scores)):.5f}")
        print(f"LR OOF Brier: {brier_score_loss(labels, probs):.5f}")
        print(f"LR OOF LogLoss: {log_loss(labels, np.clip(probs, 1e-6, 1-1e-6)):.5f}")
...
def main():
    ...
    meta_feats = np.column_stack([oof_det, oof_xg])
    ...
    meta_sub = np.column_stack([out_det["Pred"].values, out_xg["Pred"].values])
