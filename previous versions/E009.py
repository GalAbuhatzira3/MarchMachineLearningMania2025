...
def build_elo_end_features(regular: pd.DataFrame, k: float = 20.0, base: float = 1500.0, reversion: float = 0.75, hca: float = 75.0) -> pd.DataFrame:
    ...
    for season, day, winner, loser, w_score, l_score, w_loc in games.itertuples(index=False):
        ...
        if current_season is None or season != current_season:
            if current_season is not None:
                ...
                # --- NEW: Season-to-Season Carryover ---
                # Instead of wiping the dictionary, regress everyone toward the mean
                for tid in ratings:
                    ratings[tid] = (reversion * ratings[tid]) + ((1.0 - reversion) * base)

            current_season = season
            # NOTICE: We deleted `ratings = {}` here!
        ...
        if w_loc == 'H':
            winner_rating_adj += hca  # Winner was at home
        elif w_loc == 'A':
            loser_rating_adj += hca  # Winner was away (meaning Loser was at home)
        # If w_loc == 'N' (Neutral site), no adjustments are made.
        ...
        # Apply the multiplier to the standard K-factor
...
def main():
    ...
    # --------------------------------- Training --------------------------------------------------------------------- #
    # --------------------------------- ELO GRID SEARCH (TEMPORARY) ------------------------------------------------ #
    print("\n--- Running Elo Parameter Optimization ---")

    # We will test combinations of K, Mean Reversion, and Home Court Advantage
    test_ks = [15.0, 20.0, 25.0]
    test_reversions = [0.50, 0.75, 0.90]
    test_hcas = [50.0, 75.0, 100.0]
    ...
    test_model = Pipeline([("scaler", StandardScaler()), ("clf", LogisticRegression(max_iter=2000))])

    for k_val in test_ks:
        for rev_val in test_reversions:
            for hca_val in test_hcas:
                # 1. Build the Elo table for this specific combination
                temp_elo = build_elo_end_features(regular_compact, k=k_val, reversion=rev_val, hca=hca_val)

                # 2. Merge it with the base compact features & seeds
                temp_features = build_team_season_compact_features(regular_compact)
                temp_features = temp_features.merge(temp_elo, on=["Season", "TeamID"], how="left")
                temp_features = temp_features.merge(get_seeds(), on=["Season", "TeamID"], how="left")

                # 3. Handle missing seeds
                temp_features["SeedNum"] = temp_features["SeedNum"].fillna(17)
                temp_features["SeedLog"] = temp_features["SeedLog"].fillna(np.log(17))

                # 4. Build training set
                X_tmp, y_tmp, groups_tmp = build_training_from_tournament(tournament_compact, temp_features)

                # 5. Quick Cross-Validation
                briers_tmp = []
                for tr, va in gkf.split(X_tmp, y_tmp, groups=groups_tmp):
                    test_model.fit(X_tmp.iloc[tr], y_tmp[tr])
                    p = test_model.predict_proba(X_tmp.iloc[va])[:, 1]
                    briers_tmp.append(brier_score_loss(y_tmp[va], p))

                mean_brier = np.mean(briers_tmp)

                if mean_brier < best_elo_brier:
                    best_elo_brier = mean_brier
                    best_params = {'k': k_val, 'reversion': rev_val, 'hca': hca_val}
                    print(f"New Best! K={k_val}, Rev={rev_val}, HCA={hca_val} --> Brier: {mean_brier:.5f}")

    print(f"\n>> ABSOLUTE BEST ELO PARAMS: {best_params} <<\n")
    # -------------------------------------------------------------------------------------------------------------- #
